...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        mainPanel(
            ...
            plotOutput("results_plot", width = "100%", height = "auto")
        )
    ),
...
server <- function(input, output) {
    observeEvent(input$calculate, {
        ...
        output$results_plot <- renderPlot({
            df <- data.frame(Component = c("H2O", "PBS"),
                             Amount = c(h2o_amount_ul, pbs_amount_ul))
            
            ggplot(df, aes(x = Component, y = Amount)) +
                geom_bar(stat = "identity", fill = "dodgerblue") +
                labs(title = "Amount of H2O and PBS Needed",
                     x = "Component", y = "Amount (μL)") +
                theme_minimal()
        })
    })
}
